...
import json
import pandas as pd
from modelo_v2 import run_simulation, process_parameters, include_stacked_chart_diagram_for_good_quality_components, plot_results, plot_stacked_chart, plot_discarded_components_stacked_chart
...
if st.button("Run Simulation"):
    ...
    results, df_monitoring = run_simulation(simulation_time, process_parameters, generate_plots=False)

    # Extraer include_stacked_chart de process_parameters
   # include_stacked_chart = process_parameters.get("include_stacked_chart", False)

    # Si está activado, mostrar el gráfico
    #if include_stacked_chart:
    #    plot_stacked_chart(df_monitoring)

    # Pasar df_monitoring a la función de gráficos
    #plot_results(df_monitoring)
    ...
    plot_results(df_monitoring)  # ✅ Usar df_monitoring en lugar de monitoring_data
    if include_stacked_chart_diagram_for_good_quality_components == 'yes':
        plot_stacked_chart(df_monitoring)
        plot_discarded_components_stacked_chart(df_monitoring)
